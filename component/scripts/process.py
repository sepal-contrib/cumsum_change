        ...
        try:
            n = tf.cast(
                tfp.stats.percentile(
                    tf.boolean_mask(s_diff, tf.math.not_equal(s_diff, 0)), 95
                ),
                "float32",
            )
...
def bootstrap(stack, s_diff, nr_bootstraps):
    ...
    signficance = tf.math.subtract(
        1,
        tf.math.divide_no_nan(tf.math.divide_no_nan(change_sum, nr_bootstraps), s_diff),
    )
    ...
    with rio.open(infile) as src:
        ...
        with rio.open(outfile, "w", **outmeta) as dst:
            ...
            for ij, window in dst.block_windows():
                ...
                stack_tf = tf.convert_to_tensor(
                    np.nan_to_num(stack[date_index_subset]), dtype="float32"
                )
                mask = tf.convert_to_tensor(
                    np.isfinite(stack[date_index_subset]).astype("float32"),
                    dtype="float32",
                )

                # calculate mean
                mean_tf = tf.math.divide_no_nan(
                    tf.math.reduce_sum(stack_tf, axis=0),
                    tf.math.reduce_sum(mask, axis=0),
                )
                ...
                residuals = tf.where(
                    tf.math.equal(stack_tf, 0), tf.zeros_like(stack_tf), residuals
                )
...
def run_cumsum(ts_folder, outdir, tiles, period, bstraps, area_thld, conf_thld, out, alert_module=False):
    ...
    for tile in tiles:
        ...
        date_index_subset = np.where(
            (date_index > period[0]) & (date_index < period[1])
        )
